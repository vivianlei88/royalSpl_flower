...
async function generateOmniImage(params: SubmitParams): Promise<ImageResult[]> {
  // Step 1: Submit task
  const submitResp = await fetch("https://app-des7uh9iwsu9-api-2Y00Vzbe0MBY.gateway.appmedo.com/v1/images/omni-image", {
    method: "POST",
    headers: {
  ...
  while (Date.now() < deadline) {
    ...
    const queryResp = await fetch(
      `https://app-des7uh9iwsu9-api-n9QVxo8blgrL.gateway.appmedo.com/v1/images/omni-image/${taskId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
